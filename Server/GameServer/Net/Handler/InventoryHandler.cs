...
namespace Server.Handler
{
    public static class InventoryHandler
    {
        ...
        public static void PickupItem(InPacket lea, Client gc)
        {
            ...
            // 撿取靈魂
            if (dropItemId >= 9900001 && dropItemId <= 9900004)
            {
                switch (dropItemId)
                {
                    case 9900001: // Blue
                ...
                foreach (Character All in Map.Characters)
                {
                    InventoryPacket.ClearDropItem(All.Client, chr.CharacterID, OriginalID, dropItemId);
                }
                return;
            }

            // 撿取錢
            if (dropItemId >= 9800001 && dropItemId <= 9800005)
            {
            ...
            //
            short dropQuantity = Map.getDropByOriginalID(OriginalID).Quantity;
            byte Type = InventoryType.getItemType(dropItemId);
             // 合併消費物品跟其他物品(数量小于100)
            if (((Type == (byte)InventoryType.ItemType.Spend3)
                || (Type == (byte)InventoryType.ItemType.Other4))
                && (finditem != null)
                && (finditem.Quantity + dropQuantity) <= 100)
            {
                chr.Items[(InventoryType.ItemType)finditem.Type, finditem.Slot].Quantity += dropQuantity;
            }
            else
            {
                byte Slot = gc.Character.Items.GetNextFreeSlot((InventoryType.ItemType)Type);
                Item oItem = new Item(dropItemId, Type, Slot, dropQuantity);
                chr.Items.Add(oItem);
            }
            //遍历地图角色。告知其他角色该物品已消失
            foreach (Character All in Map.Characters)
