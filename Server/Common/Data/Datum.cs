...
namespace Server.Common.Data
{
    ...
    public class Datum : DynamicObject
    {
         ...
         * 根据ID查询记录是否存在
         */
        public bool existById(String fieldIdName , int id)
        {
            int count = 0;
            string whereStr = fieldIdName + "={0}";
            count = Database.Count( this.Table, fieldIdName, whereStr, id);
            return count > 0;
        }
        public void Insert()
        {
            ...
            {
                //转义避免遇到数据库关键字导致sql报错
                String signKey = "`"+ loopPair.Key.Trim()+"`";
                fields += signKey;
                processed++;
            ...
            foreach (KeyValuePair<string, object> loopPair in this.Dictionary)
            {
                ...
                String signValue = "";
                if (loopPair.Value is bool)
                {
                    signValue = ((bool)loopPair.Value) ? "1" : "0";
                }
                else
                {
                    signValue = string.Format("'{0}'", loopPair.Value);
                }
                fields += signValue;
        ...
        public void Update(string constraints, params object[] args)
        {
            ...
            {
                String signKey = "`" + loopPair.Key.Trim() + "`";
                fields += string.Format("{0}='{1}'", signKey, loopPair.Value is bool ? (((bool)loopPair.Value) ? 1 : 0) : loopPair.Value);
                processed++;
