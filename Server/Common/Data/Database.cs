...
namespace Server.Common.Data
{
    ...
    public static class Database
    {
        ...
        internal static string ConnectionString
        {
            get
            {
                String connectionStr = string.Format("server={0}; database={1}; uid={2}; password={3}; convertzerodatetime=yes; CharSet=utf8;",
                    Database.Host,
                    Database.Schema,
                    Database.Username,
                    Database.Password);
                Log.Inform(connectionStr);
        ...
        {
            String countSql = string.Format("SELECT COUNT({1}) FROM {0} WHERE {2}", table, field, string.Format(constraints, args));
            Log.Sql(countSql);
            using (MySqlDataReader reader = MySqlHelper.ExecuteReader(Database.ConnectionString, countSql))
            {
                if (reader.Read())
                {
